...
We can define the computation first which can make debugging easier if needed. 

```{r}
compute_group_chull <- function(data){

    chull_rows <- chull(data$x, data$y)

    data[chull_rows, , drop = FALSE]

  }
```

Then, we'll use this group computation to define a StatChull.
...
And we use StatChull to produce a ggplot using the layer function.
...
ggplot(mpg, aes(displ, hwy)) + 
  geom_point() + 
  layer(geom = "polygon", stat = StatChull, position = "identity")
```


But we have not given the user a familiar and concise way to use it - users typically don't use layer directly.
